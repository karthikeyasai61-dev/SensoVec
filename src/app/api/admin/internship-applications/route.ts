...
export async function GET(request: Request) {
  try {
    ...
    console.error("Admin list internship applications error:", error);
...
export async function PATCH(request: Request) {
  try {
    ...
    const appRef = adminDb.collection("internship_applications").doc(applicationId);
    ...
    const courseTitle = appData.courseTitle || "Internship Program";
        ...
        if (status === "approved_for_details") {
          ...
          htmlContent = `
            <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; padding: 30px; border: 1px solid rgba(255,255,255,0.08); border-radius: 16px; background-color: #0c0e17; color: #f8fafc; box-shadow: 0 10px 30px rgba(0,0,0,0.5);">
              ...
              <p style="font-size: 1rem; line-height: 1.6; margin-bottom: 20px; color: #cbd5e1;">
                Great news! We have reviewed your application and are pleased to inform you that your resume has been approved for the next stage of the <strong>${courseTitle}</strong> internship program at <strong>SensoVec</strong>.
              </p>
              ...
              <div style="font-size: 1rem; line-height: 1.6; margin-bottom: 24px; background: rgba(0, 136, 255, 0.08); border: 1px solid rgba(0, 136, 255, 0.2); padding: 16px; border-radius: 10px; color: #38bdf8;">
                ...
                Please log in to your account, visit the internship course page, and fill out the additional details required to proceed. After submitting these details, you will be able to complete your enrollment.
              </div>
          ...
          htmlContent = `
            <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; padding: 30px; border: 1px solid rgba(255,255,255,0.08); border-radius: 16px; background-color: #0c0e17; color: #f8fafc; box-shadow: 0 10px 30px rgba(0,0,0,0.5);">
              ...
              <p style="font-size: 1rem; line-height: 1.6; margin-bottom: 20px; color: #cbd5e1;">
                Thank you for your interest in the <strong>${courseTitle}</strong> internship program at <strong>SensoVec</strong>.
              </p>
              
              <p style="font-size: 1rem; line-height: 1.6; margin-bottom: 24px; color: #cbd5e1;">
                We have reviewed your application and resume. Unfortunately, we are unable to accept your application for this internship cohort. We receive many applications and are only able to proceed with candidates whose qualifications and experience closely align with our current needs.
              </p>
    ...
    console.error("Admin internship application review error:", error);
