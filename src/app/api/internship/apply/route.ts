...
export async function POST(request: Request) {
  try {
    ...
    // Check if an application already exists for this student and internship course
    const existingApps = await adminDb.collection("internship_applications")
    ...
    if (!existingApps.empty) {
      ...
      if (appData.status === "enrolled") {
        return NextResponse.json({ error: "Already enrolled in this internship" }, { status: 400 });
      }
      ...
      applicationRef = adminDb.collection("internship_applications").doc();
      applicationData = {
        studentId,
        studentName: sscFullName,
    ...
    try {
          ...
          html: `
            <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; padding: 30px; border: 1px solid rgba(255,255,255,0.08); border-radius: 16px; background-color: #0c0e17; color: #f8fafc; box-shadow: 0 10px 30px rgba(0,0,0,0.5);">
              ...
              <p style="font-size: 1rem; line-height: 1.6; margin-bottom: 20px; color: #cbd5e1;">
                Thank you for choosing <strong>SensoVec</strong> for your internship journey. We have successfully received your application for the <strong>${courseTitle}</strong> internship.
              </p>
      ...
      // We do not block the client application completion even if email delivery fails
    }
