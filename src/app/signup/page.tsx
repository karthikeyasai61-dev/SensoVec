...
export default async function SignupPage({ searchParams }: { searchParams: Promise<{ redirect?: string }> }) {
  ...
  if (isStudent || isAdmin) redirect(redirectUrl || "/");
  ...
  return (
    <div className="auth-page-wrapper" style={{ display: "flex", minHeight: "100vh", padding: "2rem 1rem", position: "relative" }}>
      ...
      <Suspense fallback={
        <div className="glass-card" style={{ padding: "3rem 2rem", margin: "auto", display: "flex", justifyContent: "center", alignItems: "center" }}>
          <LogoSpinner size={50} padding="0" />
        </div>
      }>
        <SignupClient />
      </Suspense>
