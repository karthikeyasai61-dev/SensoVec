...
export default async function ProfilePage() {
  ...
  const enrollments = await Promise.all(enrollmentsQuery.docs.map(async (doc) => {
    ...
    const isCompleted = data.completed === true || courseData?.isCompleted === true;
    return {
      id: doc.id,
      ...data,
              ...
              <div key={e.id} className="glass-card" style={{ padding: "1.25rem" }}>
                {e.course.imageUrl && (
                  ...
                  <img src={e.course.imageUrl} alt={e.course.title}
                    style={{ width: "100%", height: "120px", objectFit: "contain", backgroundColor: "rgba(0,0,0,0.2)", borderRadius: "8px", marginBottom: "0.75rem" }} />
                )}
            ...
            {completedCourses.map((e: any) => (
              <div key={e.id} className="glass-card" style={{ padding: "1.25rem", opacity: 0.8 }}>
                {e.course.imageUrl && (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={e.course.imageUrl} alt={e.course.title}
                    style={{ width: "100%", height: "120px", objectFit: "contain", backgroundColor: "rgba(0,0,0,0.2)", borderRadius: "8px", marginBottom: "0.75rem", filter: "grayscale(30%)" }} />
                )}
